...
const getMethodAndPathSchema = (api, regexPathDefinitions, path, method) => {
...
module.exports = (api, regexToPathMapping) => {
    return {
        getRequestParameters: (path, method) => {
            return getMethodAndPathSchema(api, regexToPathMapping, path, method).parameters || [];
        },
        getResponseStatus: (path, method, status) => {
            const pathWithMethod = getMethodAndPathSchema(api, regexToPathMapping, path, method);
