...
module.exports = (schemaRetriever, pathMatcher) => {
    return (apiPath, apiMethod, parameters) => {
        ...
        const schemaParameterDefinitions = schemaRetriever.getRequestParameters(matchedPathObj.path, lowerMethod);
        ...
        if (paramsSchema.path) {
            const pathParams = pathMatcher.match(apiPath)
            const ajv = new Ajv({
                coerceTypes: true,
            })
            if (!ajv.validate(paramsSchema.path, pathParams.params)) {
                throw ajv.errors
            }
        }
